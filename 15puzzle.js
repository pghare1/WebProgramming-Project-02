...
function shiftPos(toLoc, fromLoc) {
  ...
  if (!solved) {
    ...
    console.log('toLoc.x;',toLoc.x)
    toLoc.x = fromLoc.x;
    
    console.log('fromLoc.x;',fromLoc.x)
    toLoc.y = fromLoc.y;
    
    console.log('fromLoc.y;', fromLoc.y)
    checkSolved();
  }
}
...
async function LerpTile (to, from)
{ 
  for (const v of lerp(Number(from), Number(to), 0.33))
